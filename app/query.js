...
function registerAccount(username = "", password = "", type){
    ...
    pool.query(`INSERT INTO accounts(username, password, type) VALUES($1, $2, $3)`, [username, password, type]).then(result => {
  ...
  function updateUsername(userid, username){
    ...
    pool.query(`UPDATE accounts SET username = $1 WHERE usrID = $2`, [username, userid]).then(result => {
  ...
  function updatePassword(userid, password){
    ...
    pool.query(`UPDATE accounts SET password = $1 WHERE usrID = $2`, [password, userid]).then(result => {
  ...
  function addClass(crn = "", dept = "", title = "", profID = "", code = ""){
      ...
      pool.query(`INSERT INTO classes(crn, department, title, professorid, registrationcode) VALUES($1, $2, $3, $4, $5)`, [crn, dept, title, profID, code]).then(result =>{
  ...
  function updateTitle(crn, title = ""){
    ...
    pool.query(`UPDATE classes SET title = $1 WHERE crn = $2`, [title, crn]).then( result => {
  ...
  function updateProfessor(crn, profID =  ""){
    ...
    pool.query(`UPDATE classes SET professorid = $1 WHERE crn = $2`, [profID, crn]).then( result => {
  ...
  function updateRegCode(crn, code = ""){
    ...
    pool.query(`UPDATE classes SET registrationcode = $1 WHERE crn = $2`, [code, crn]).then( result => {
  ...
  function getAccounts(type = 0){
    ...
    if(type != 0){
      pool.query(`SELECT * FROM accounts WHERE type = $1`, [type]).then(result => {
  ...
  function getClasses(department = "", profID = 0){
    ...
    if(profID === 0){
      pool.query(`SELECT * FROM classes WHERE department = $1`, [department]).then(result => {
        console.log(result.rows);
        return result.rows;
      })
    }
    //search by professor 
      ...
      pool.query(`SELECT * FROM classes WHERE professorID = $1`, [profID]).then(result => {
        console.log(result.rows);
        return result.rows;
      })
    }
    ...
    //search by both department and professor
      pool.query(`SELECT * FROM classes WHERE department = $1 AND professorID = $2`, [department, profID]).then(result => {
        console.log(result.rows);
        return result.rows;
      })
    }
  ...
  function enroll(userid, regCode){
    pool.query(`SELECT crn FROM classes WHERE registrationcode = $1`, [regCode]).then(result => {
      let crn = result.rows;
      console.log("crn:" + crn);
